<html>
  <head>
    <title>SJTU-Schemer Alpha</title>
...
function toHex(n){
    ...
    return (result==''?'0':result);
      ...
      $(document).ready(function(){
	   ...
	   if (!request)
         ...
         var console2 = $('<div class="console1">');
         $('body').append(console2);
         var controller2 = console2.console({
           promptLabel: 'SJTU-Schemer>>> ',
	   	...
	   	var a=line.match(/\(/g),b=line.match(/\)/g);
		var la=a?a.length:0;
		var lb=b?b.length:0;
		//controller2.report = report
		if(la<= lb){
                 	controller2.continuedPrompt = false;
			cmdRef=cmdRef+'\n'+line;
			//alert(cmdRef);
		       /*try { var ret = eval(line);
			     if (typeof ret != 'undefined') return [{msg:ret.toString(),className:"jquery-console-message-value"}];

			     else return true; }
		       catch (e) { return e.toString(); }*/
						 var url = "/eval2json/"//+encodeHex(cmdRef);
						 //alert(url);
			 ...
			 request.onreadystatechange = function () {
				 ...
				 //alert(result);
				 report( [{msg:result,className:"jquery-console-message-value"}]);
			       } else
				 alert("status is " + request.status);
			     }
			   }
			request.setRequestHeader("Content-type", "application/x-www-form-urlencoded")
		     	 request.send("code="+encodeHex(cmdRef));
			 /*while(1){
				 if(result != "")break;
				 //sleep(10);
				 setTimeout("3*4",300);
           ...
           animateScroll:true,
           promptHistory:true,
           welcomeMessage:'Enter some Scheme expressions to evaluate.'
         });
         controller2.promptText('(+ 3 4) ');
    ...
    <style type="text/css" media="screen">
      ...
      div.console1 div.jquery-console-inner 
       { width:900px; height:500px; background:#333; padding:0.5em;
         overflow:auto }
  ...
  <body>
    ...
    <div id="content">
	    <h1>SJTU-Schemer 0.1 Alpha by Liuexp.</h1>
    </div>
<br>
